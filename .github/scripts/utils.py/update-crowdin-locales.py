...
def get_updated_locales() -> list[str]:
    COMMITS_TO_CHECK = f"{os.environ['GITHUB_EVENT_BEFORE']}..{os.environ['GITHUB_SHA']}"
    updated_locales = subprocess.check_output(["git", "diff", "--name-only", COMMITS_TO_CHECK])
...
def upload(updated_locales: list[str], submodule_name: str = None) -> None:
    ...
    for file_path in updated_locales:
        ...
        if submodule_name is not None and file_path == f"data/locale/{DEFAULT_LOCALE}.ini":
            submodule_export_path = f"plugins/{submodule_name}/data/locale/{DEFAULT_LOCALE}.ini"
            if submodule_export_path in export_paths_map:
                update_crowdin_file(export_paths_map[submodule_export_path], file_path)
            else:
                logger.error(f"Unable to find {submodule_export_path} on Crowdin.")
            break
...
if len(updated_locales) != 0:
    if "SUBMODULE_NAME" in os.environ:
        upload(updated_locales, os.environ["SUBMODULE_NAME"])
    else:
